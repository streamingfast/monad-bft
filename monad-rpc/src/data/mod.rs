...
pub async fn try_get_block_key_from_tag_or_hash(
    triedb_env: &impl Triedb,
    block_reference: BlockTagOrHash,
) -> Result<Option<BlockKey>, ChainStateError> {
    match block_reference {
        BlockTagOrHash::BlockTags(tag) => Ok(get_block_key_from_tag(triedb_env, tag)),
        BlockTagOrHash::Hash(block_hash) => {
            let latest_block_key = triedb_env.get_latest_proposed_block_key();

            let Some(block_number) = triedb_env
...
pub async fn get_block_key_from_tag_or_hash(
    ...
    try_get_block_key_from_tag_or_hash(triedb_env, block_reference)
        .await
        .ok()
        .flatten()
}
    ...
    T: Triedb,
{
    ...
    pub async fn get_logs(
                ...
                let block_num = match block {
                    ...
                    None => {
                            ...
                            .map_err(|e| JsonRpcError::internal_error(e.to_string()))?
                        {
                            ...
                            // to block numbers.
                            match block_pointer {
                                BlockPointer::Finalized(block_num) => block_num,
                                BlockPointer::NonFinalized(_, _) => {
                                    return Err(JsonRpcError::internal_error(
                                        "archive produced non-finalized block number after resolving block hash".to_string()
                                    ));
                                }
                            }
                        } else {
        ...
        let logs_stream = stream_with_archive.map({
            let filtered_params = &filtered_params;

            move |result| {
                result.and_then(|(header, transactions, receipts)| {
                    header_transactions_receipts_to_block_number_logs(
                        header,
                        transactions,
                        receipts,
                        filtered_params,
...
fn receipt_envelope_to_logs_iter<'a>(
...
async fn fetch_bloom_filtered_header_transactions_receipts_from_archive(
    ...
    Ok((header, transactions, bloom_receipts))
}
            ...
            .map_ok(move |(block_number, receipts)| {
                (
                    ...
                    receipts.into_iter().flat_map(move |receipt| {
                        receipt_envelope_to_logs_iter(receipt, filtered_params)
                    }),
                )
            }),
    )
...
pub fn parse_tx_receipt(
    ...
    let inner = parse_receipt_envelope(
        block_hash,
        block_number,
        block_timestamp,
        tx_index,
    ...
    TransactionReceipt {
        ...
        gas_used,
        // effective gas price is calculated according to eth json rpc specification
        effective_gas_price: tx.effective_gas_price(block_base_fee_per_gas),
        // TODO: EIP4844 fields
        blob_gas_used: None,
        blob_gas_price: None,
    }
...
fn header_transactions_receipts_to_block_number_logs<'a>(
    header: BlockHeader,
    transactions: Vec<TxEnvelopeWithSender>,
    receipts: Vec<ReceiptWithLogIndex>,
    filtered_params: &'a FilteredParams,
    ...
    if transactions.len() != receipts.len() {
        return Err(JsonRpcError::internal_error(
            "transactions and receipts length does not match".to_owned(),
        ));
    }

...
pub(crate) struct StateSyncClient<PT: PubKey> {
    ...
    progress: Arc<Mutex<StateSyncProgress>>,
...
impl<PT: PubKey> StateSyncClient<PT> {
    pub fn start(
        ...
        let progress = Arc::new(Mutex::new(StateSyncProgress::default()));
...
impl<PT: PubKey> Stream for StateSyncClient<PT> {
